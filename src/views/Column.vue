<template>
  <div class="successDemo">
    ...
    <detail-nav
      :navtitle="title"
      :navTitleChild="columnTitleList[demoId]"
    ></detail-nav>
    <div class="success-demo-box">
      <!-- 左边侧边栏 -->
      <left-tab
        :leftTitle="title"
        :LeftList="columnTitleList"
        :showLeft="demoId"
      ...
      <div class="success-demo-right">
        <div class="newscontent">
          <div>
            <nav>
              <span>{{ columnTitleList[demoId] }}</span>
            </nav>
            <ul class="demo-list">
              <li
                class="demo-item"
                @click="toSuccessDetail(item)"
                v-for="(item, index) in columnList[demoId]['totalList'][
                  currentPage - 1
                ].list"
                :key="index"
              >
...
import { COLUMN_LIST } from "../assets/data/columnData";
...
export default {
  ...
  mounted() {
    ...
    NAVIGATORList[this.showLeft].subNav.forEach((item) => {
      this.columnTitleList.push(item.label);
    });
  },
  methods: {
    // 左侧点击
