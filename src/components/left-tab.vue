<template>
  <div class="detail-left">
    <div>
      <div class="navlefttitle">{{leftTitle}}</div>
      <ul class="leftList">
        <li
          @click="handleClickLeft(index)"
          :class="{ _activeli: showLeft == index }"
...
<script>
export default {
  props: ['leftTitle','LeftList','showLeft'],
  methods:{
    handleClickLeft(index) {
      this.$emit('handleClickLeft',index)
    }
  }
}
</script>
...
.detail-left {
  ...
  $pd-lf: padding-left;
  .navlefttitle {
    background: url("../assets/images/detail/newsnav_img1.png");
  ...
  .leftList {
    @extend %ulst;
    margin-top: 10px;
    border-bottom: 4px solid #104c8b;
    padding-bottom: 24px;
    width: 100%;
    background: #ffffff;
    & > li {
